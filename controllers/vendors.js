...
export const registerVendor = async (req, res, next) => {
  try {
    ...
    await mailTransporter.sendMail({
      ...
      text: "Account registered! Postiize...post with ease",
    });
...
export const loginVendor = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id: vendor.id }, process.env.JWT_PRIVATE_KEY, {
      expiresIn: "24h",
    });
    // respond to request
    res.status(200).json({
...
export const getProfile = async (req, res, next) => {
  try {
    ...
    const vendor = await VendorModel.findById(req.auth.id).select({
      password: false,
    });
    // respond to request
    res.json(vendor);
